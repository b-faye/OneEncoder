...
def zero_shot_classification(model, dataloader, unique_class_names, temperature=2.5):
    ...
    with torch.no_grad():
        for id, batch in enumerate(tqdm(dataloader, desc="Testing")):
            ...
            audio_probs = (101.0 * similarities).softmax(dim=-1)
...
def main(model_path="audio_image.pt"):
    ...
    test_set = torchvision.datasets.Caltech101(root='./data', download=True)
    class_names = test_set.categories
    test_set = CustomCaltech101Dataset(test_set, transform=model.image_preprocessor)
