...
class LinearClassifier(nn.Module):
    def __init__(self, pretrained_model, num_classes=50, trainable=False, device='cpu', *args, **kwargs):
...
def main(model_path="audio_image.pt", trainable=False, epochs=10):
    ...
    esc = load_dataset("ashraq/esc50", trust_remote_code=True)
    esc = esc.shuffle()
    # Manually split the dataset
    total_size = len(esc['train'])
    val_size = int(0.2 * total_size)
    train_size = total_size - val_size
    train_data = esc['train'].select(range(train_size))
    test_data = esc['train'].select(range(train_size, total_size))
