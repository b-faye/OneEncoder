...
def main(model_path="text_image.pt", trainable=False, epochs=100, device=CFG.device):
    ...
    train_data = CustomOxfordIIITDataset(train_data, transform=pretrained_model.image_preprocessor)
    test_data = torchvision.datasets.OxfordIIITPet(root='./data', split='test', download=True)
    test_data = CustomOxfordIIITDataset(test_data, transform=pretrained_model.image_preprocessor)
