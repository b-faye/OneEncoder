...
def zero_shot_classification(model, dataloader, unique_class_names, temperature=2.0):

    accuracy = {f"top_{i+1}_accuracy": 0 for i in range(5)}
    # Encode label names
    text_features = model.encode_text(texts=[f"a {name}, this is a photo of a {name}" for name in unique_class_names])
