...
class LinearClassifier(nn.Module):
    def __init__(self, pretrained_model, num_classes=100, trainable=False, device='cpu', *args, **kwargs):
        ...
        self.classifier = nn.Sequential(
            # self.model.head.in_features = 1024
...
def main(trainable=True, epochs=100, device=CFG.device):
    ...
    pretrained_model.head.fc = nn.Identity()   # Remove the final fully connected layer
    # Load Data
    train_data = torchvision.datasets.CIFAR100(root='./data', train=True, download=True)
    train_data = CustomCIFAR100Dataset(train_data, transform=text_image_encoder.image_preprocessor)
    test_data = torchvision.datasets.CIFAR100(root='./data', train=False, download=True)
    test_data = CustomCIFAR100Dataset(test_data, transform=text_image_encoder.image_preprocessor)
    # DataLoader
    ...
    torch.save(model.state_dict(), 'swin_transformer_linear_classifier.pt')
