...
class CustomCaltech101Dataset(Dataset):
...
class LinearClassifier(nn.Module):
    def __init__(self, pretrained_model, num_classes=101, trainable=False, device='cpu', *args, **kwargs):
        ...
        self.classifier = nn.Sequential(
            # self.pretrained_model.fc.in_features = 512
            nn.Linear(512, 512),
...
def main(trainable=True, epochs=100, device=CFG.device):
    ...
    train_data = CustomCaltech101Dataset(train_data, transform=text_image_encoder.image_preprocessor)
    test_data = CustomCaltech101Dataset(test_data, transform=text_image_encoder.image_preprocessor)
    # DataLoader
    ...
    torch.save(model.state_dict(), 'resnet_linear_classifier.pt')
