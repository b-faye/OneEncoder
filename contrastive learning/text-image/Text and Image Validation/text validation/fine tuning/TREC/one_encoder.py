...
class TREC_Dataset(Dataset):
    ...
    def __getitem__(self, idx) -> Tuple[str, int]:
        ...
        label = self.data[idx]['coarse_label']  # Assuming you want the coarse label
        return text, label

def collate_fn(batch) -> Tuple[List[str], torch.Tensor]:
    texts, labels = zip(*batch)
    return list(texts), torch.tensor(labels)

class Classifier(nn.Module):
    def __init__(self, pretrained_model: TextImageEncoder, hidden_size=CFG.projection_dim, num_labels=6):
...
def main():
    ...
    train_dataset = TREC_Dataset(trec['train'])
    test_dataset = TREC_Dataset(trec['test'])
    ...
    model = Classifier(pretrained_model, num_labels=6)
