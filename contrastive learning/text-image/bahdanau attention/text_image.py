...
class UniversalProjectionEncoder(nn.Module):
    def __init__(self, input_dim=CFG.projection_dim, num_head=CFG.num_head, num_layers=CFG.num_layers, *args, **kwargs):
        ...
        self.bahdanau_attention = BahdanauAttention(projection_dim=self.input_dim)
    ...
    def forward(self, inputs):
        ...
        attention = self.bahdanau_attention([x, tokens])
...
def main(basic_train=False):
    ...
    # CoCo size 413.915 captions --- Flirk size 158.915 captions  --- TextCaps 109.765 = 682.595
    coco_training_pairs, flirk_training_pairs, text_caps_training_pairs = make_pairs(CFG.train_annotation_file,
                                                                                     CFG.image_dir, 5)

    # coco validation 202.520 + 15.830 = 218.350
    coco_validation_pairs, _, text_caps_validation_pairs = make_pairs(CFG.val_annotation_file, CFG.image_dir_val, 5,
                                                                      train=False)

    # 682595 + 218350 = 900.945
    all_pairs = coco_training_pairs + flirk_training_pairs + text_caps_training_pairs + coco_validation_pairs + text_caps_validation_pairs
    random.shuffle(all_pairs)

    training_pairs = all_pairs[:-150000]
    validation_pairs = all_pairs[-150000:]
    ...
    image_embeddings = get_image_embeddings(coco_training_pairs, "best.pt")
    torch.save(image_embeddings, "image_embeddings_best.pt")
    caption_embeddings = get_caption_embeddings(coco_training_pairs, "best.pt")
