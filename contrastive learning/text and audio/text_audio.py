...
class TextAudioEncoder(nn.Module):
    def __init__(self, device='cpu', modality_token_encoder=ModalityTokenEncoder(), checkpoint="../text_image_add/text_image.pt",
        ...
        super(TextAudioEncoder, self).__init__(*args, **kwargs)
    ...
    def process_audio(self, audios):
        ...
        x = self.audio_processor(audios, sampling_rate=self.sample_rate, return_tensors="pt", padding=True, max_length=10*self.sample_rate, truncation=True)
    ...
    def forward(self, inputs):
        ...
        audios = self.audio_processor(audios, sampling_rate=self.sample_rate, return_tensors="pt", padding=True, max_length=10*self.sample_rate, truncation=True)
...
def main(basic_train=True):
    ...
    model = TextAudioEncoder(device=CFG.device)
