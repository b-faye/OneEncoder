...
from text_radio import OneEncoder import TextRadioEncoder
...
class LinearClassifier(nn.Module):
    def __init__(self, pretrained_model, num_classes=14, trainable=False, device='cpu', *args, **kwargs):
...
def main(model_path="text_radio.pt", trainable=True, epochs=100, device=CFG.device):
    ...
    dataset = load_dataset("alkzar90/NIH-Chest-X-ray-dataset", "image-classification",
                           trust_remote_code=True, token="")
    train_data = CustomChexpertDataset(dataset['train'], pretrained_model.radio_processor)
    test_data = CustomChexpertDataset(dataset['test'], pretrained_model.radio_processor)
