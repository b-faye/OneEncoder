...
from text_radio import OneEncoder as  TextRadioEncoder
import pandas as pd
...
def load_rsna_dataset():
    ...
    df = pd.read_csv(os.path.join(path, "stage_2_train_labels.csv"))
    dataset = []
    for id, row in df.iterrows():
        image_path = os.path.join(path, "stage_2_train_images", row['patientId'])
...
class CustomRSNADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pillow_image = Image.fromarray(dicom_data.pixel_array)
        image = self.transform(pillow_image, return_tensors="pt")['pixel_values'].squeeze(0)
...
class LinearClassifier(nn.Module):
    def __init__(self, pretrained_model, num_classes=2, trainable=False, device='cpu', *args, **kwargs):
    ...
    def forward(self, inputs):
        radio_features = self.model.encode_radio(radios=inputs)
...
def main(model_path="text_radio.pt", trainable=False, epochs=100, device=CFG.device):
    pretrained_model = TextRadioEncoder(device=device)
    ...
    train_data = CustomRSNADataset(train_data, pretrained_model.radio_processor)
    test_data = CustomRSNADataset(test_data, pretrained_model.radio_processor)
