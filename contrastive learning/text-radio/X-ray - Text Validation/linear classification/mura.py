...
class CustomMuraDataset(Dataset):
...
def main(model_path="text_radio.pt", trainable=False, epochs=100, device=CFG.device):
    ...
    dataset = load_dataset("KhalfounMehdi/MURA", trust_remote_code=True, token="")
    ...
    test_data = dataset['train'].select(range(train_size, total_size))
    train_data = CustomMuraDataset(train_data, pretrained_model.radio_processor)
    test_data = CustomMuraDataset(test_data, pretrained_model.radio_processor)
